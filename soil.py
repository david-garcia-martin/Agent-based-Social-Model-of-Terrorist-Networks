...
def visualization(graph_name):
    ...
    for node in range(settings.network_params["number_of_nodes"]):
        G.node[node]['x'] = G.node[node]['pos'][0]  # Añadimos coordenada x
        G.node[node]['y'] = G.node[node]['pos'][1]  # Añadimos coordenada y
        G.node[node]['viz'] = {"position": {"x": G.node[node]['pos'][0], "y": G.node[node]['pos'][1], "z": 0.0}}  # Añadimos un objeto posición para visualizarlo
        del (G.node[node]['pos'])  # Eliminamos el anterior parámetro de posición
